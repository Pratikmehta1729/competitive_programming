...
{   
	...
	while(t--)
	{
     ...
     //{
      ...
      while(i!=n)
      {
      	if( arr[i]>0){a=arr[i]; if(arr[i]==arr[i-1] && i>0){ arr[i-1]%=3; for(int j=i-2; j>=0; j--){if(arr[j+1]==1){arr[j]=2;}else{arr[j]=1;}}}  i++; continue;}
        ...
        if(i==0){arr[0]=1;}
        else {if(arr[i-1]==1){arr[i]=2;}else{arr[i]=1;}}
